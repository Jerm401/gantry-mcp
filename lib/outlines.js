...
async function openOutlines(arg1, arg2) {
  ...
  // A login-page redirect means the Joomla session expired.
  if (/name="username"|id="mod-login-username"|task=login/i.test(res.body)) {
    throw new Error('Session expired or CSRF rejected — got login page instead of outlines');
  }
...
function parseOutlinesFromHtml(html) {
  ...
  while ((m = cardRe.exec(html))) {
    ...
    const editAnchor = card.match(/<a\b[^>]*\bbutton-primary\b[^>]*>/);
    const layoutHref = editAnchor
      ? ((editAnchor[0].match(/\bhref="([^"]+)"/) || [])[1] || '').replace(/&amp;/g, '&')
      : '';
    const dupeAnchor = card.match(/<a\b[^>]*\bdata-g5-outline-duplicate\b[^>]*>/);
    const duplicateHref = dupeAnchor
      ? ((dupeAnchor[0].match(/\bhref="([^"]+)"/) || [])[1] || '').replace(/&amp;/g, '&')
      : '';
    const deleteHref =
...
async function _duplicateOutlineHttp(ctx, id, opts) {
  ...
  if (!parsed) {
    throw new Error(`Duplicate returned non-JSON (session expired or CSRF rejected): ${res.body.slice(0, 200)}`);
  }
...
async function _deleteOutlineHttp(ctx, id) {
  ...
  if (!j) {
    throw new Error(`Delete returned non-JSON (session expired or CSRF rejected): ${res.body.slice(0, 200)}`);
  }
  if (j.success === false) throw new Error(`Delete failed: ${j.message || res.body.slice(0, 300)}`);
  return id;
}
